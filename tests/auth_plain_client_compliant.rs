...
fn setup_their_rep(bind_endpoint: &str) -> (zmq::Socket, String, zmq::Socket) {
    ...
    std::thread::spawn(move || loop {
        ...
        let (version, sequence, mechanism, username, password) =
            (&*rq[0], &*rq[1], &*rq[5], &*rq[6], &*rq[7]);

        assert_eq!(version, b"1.0");
        assert_eq!(mechanism, b"PLAIN");
        println!("Got auth request");
        if username == PLAIN_USERNAME.as_bytes() && password == PLAIN_PASSWORD.as_bytes() {
    ...
    their_rep
        ...
        .expect("Failed to set plain server flag");
...
async fn setup_our_req(bind_endpoint: &str) -> zeromq::ReqSocket {
    ...
    sock_opts.auth_method(
        AuthMethod::plain_client(PLAIN_USERNAME, PLAIN_PASSWORD)
...
#[async_rt::test]
async fn test_auth_plain_client() {
