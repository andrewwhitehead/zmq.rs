...
const PLAIN_USERNAME: &str = "admin";
const PLAIN_PASSWORD: &str = "secret";
...
fn setup_their_rep(bind_endpoint: &str) -> (zmq::Socket, String, zmq::Socket) {
    ...
    their_rep
        .set_plain_username(Some(PLAIN_USERNAME))
        .expect("Failed to set plain username value");
    their_rep
        .set_plain_password(Some(PLAIN_PASSWORD))
        .expect("Failed to set plain password value");
...
async fn setup_our_req(bind_endpoint: &str) -> zeromq::ReqSocket {
    ...
    sock_opts.auth_method(
        AuthMethod::plain_server(|req| async move {
            match req {
                AuthRequest::Plain { username, password }
                    if username == PLAIN_USERNAME && password == PLAIN_PASSWORD =>
                {
                    true
                }
                _ => false,
        ...
        .expect("Failed to initialize plain auth options"),
    );
...
#[async_rt::test]
async fn test_auth_plain_server() {
