...
fn setup_their_rep(bind_endpoint: &str) -> (zmq::Socket, String, zmq::Socket) {
    ...
    their_rep
        .set_curve_publickey(&REP_CURVE_KEY_PUBLIC[..])
        .expect("Failed to set curve public key");
    their_rep
        .set_curve_serverkey(&REQ_CURVE_KEY.public_key)
        .expect("Failed to set curve server key");
...
async fn setup_our_req(bind_endpoint: &str) -> zeromq::ReqSocket {
    ...
    sock_opts.auth_method(
        AuthMethod::curve_server(REQ_CURVE_KEY, |req| async move {
            req == AuthRequest::Curve {
                public_key: REP_CURVE_KEY_PUBLIC,
            }
        })
...
#[async_rt::test]
async fn test_auth_curve_server() {
